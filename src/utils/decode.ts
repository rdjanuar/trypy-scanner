...
export function validateEmvcoQR(qr: string): EmvcoResult {
  if (!qr || qr.length < 10) return { valid: false, reason: 'QR kosong / terlalu pendek' }

  const parsed = parseTLV(qr)

  if (!parsed['00']) return { valid: false, reason: 'Payload indicator tidak ada' }

  if (!parsed['63']) return { valid: false, reason: 'CRC tag tidak ada' }

  // if (!validateCRC(qr)) return { valid: false, reason: 'CRC tidak valid' }

  if (parsed['58'] && parsed['58'] !== 'ID') return { valid: false, reason: 'Bukan QR Indonesia' }
