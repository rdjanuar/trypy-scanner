...
export class QrisScanner extends TwLitElement {
  ...
  firstUpdated() {
    ...
    if (window.wx?.miniProgram) {
      ...
      logger.add('DEBUG', 'WX or miniProgram is undefined on first render')
    }
    setTimeout(() => {
      this.startCamera()
    }, 300)
  ...
  connectedCallback() {
    ...
    if (window.wx?.miniProgram?.onWebviewEvent) {
      try {
        ...
        logger.add('ERROR', `Failed to register onWebviewEvent: ${err}`)
      }
    } else {
      logger.add('ERROR', 'window.wx.miniProgram is undefined in connectedCallback')
    }
  }
  ...
  disconnectedCallback() {
    ...
    this.stopCamera()
    if (this.isEventRegistered && window.wx?.miniProgram?.offWebviewEvent) {
      try {
        window.wx.miniProgram.offWebviewEvent(this.onWebViewEventEmit)
      } catch (err) {
        logger.add('ERROR', `Failed to unregister offWebviewEvent: ${err}`)
      }
    ...
    try {
      ...
      try {
        ...
        if (fallbackErr instanceof Error && fallbackErr.name === 'NotAllowedError') {
          ...
          this.errorMessage =
            'Akses kamera ditolak. Mohon izinkan akses kamera di pengaturan browser Anda, lalu muat ulang halaman.'
  ...
  protected render() {
    return html`
      ...
      ${this.isCameraDenied
        ? html`
            <div
              class="absolute inset-0 z-2 bg-strong flex flex-col items-center justify-center p-6 text-center pointer-events-auto"
            >
              <div class="size-12 rounded-full bg-white/10 flex items-center justify-center mb-3">
                <svg class="size-6 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path
                    ...
                    stroke-width="2"
                    d="M3 3l18 18M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894l-2.023-1.011M2 12v5a2 2 0 002 2h11M13 7h-2M7 7H4a2 2 0 00-2 2v2"
                  />
                </svg>
              </div>
              <p class="text-white font-medium text-sm mb-1">Akses kamera ditolak</p>
              <p class="text-white/70 text-xs px-8 mb-4">
                Mohon izinkan akses kamera secara manual melalui pengaturan browser Anda.
              </p>
              <button
                class="text-white font-semibold text-xs underline bg-transparent border-none cursor-pointer"
                @click=${this.retryCamera}
              >
                Muat Ulang Halaman
              </button>
            </div>
      ...
      ${this.status === 'idle'
        ? html`
            <div
              class="absolute top-0 inset-x-0 h-80 bg-linear-to-b from-black to-transparent pointer-events-none z-1"
            ></div>
          `
        : ''}
      ...
      <div class="absolute inset-0 z-2 flex flex-col pointer-events-none">
        ...
        <div
          class="absolute inset-x-0 px-4 flex justify-between pointer-events-auto z-3"
          style="bottom: ${this.bottomSheetHeight + 16}px"
        >
          <button
            class="p-3 rounded-full bg-white flex items-center justify-center shadow-lg border-none cursor-pointer text-gray-700"
            @click=${this.toggleFlash}
          >
            <img src=${dayIcon} />
          </button>
          <button
