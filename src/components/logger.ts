  ...
  protected render() {
    return html`
      ...
      >
        <div
          slot="header"
          class="flex justify-between items-center py-2 px-3 bg-white/5 border-b border-zinc-800 font-mono"
        >
          <span class="text-[#4fc3f7] text-[13px] font-bold">📋 Debug Logs (${this.logs.length})</span>
          <div class="flex gap-2">
            <button
              class="border-none py-1 px-2.5 rounded text-[11px] cursor-pointer text-white bg-zinc-800 hover:bg-zinc-700"
              @click=${this.clear}
            >
              Clear
            </button>
            <button
              class="border-none py-1 px-2.5 rounded text-[11px] cursor-pointer text-white bg-red-500 hover:bg-red-600"
              @click=${this.close}
            >
              ✕
            </button>
        ...
        <div class="flex-1 overflow-y-auto log-list font-mono h-full">
          ${map(
            ...
            (log) => html`
              <div
                class="group flex items-start gap-1 py-1 px-2 border-b border-white/5 text-[11px] leading-[1.4] break-all"
              >
                <div class="flex-1">
